...
Exec.prototype = {
	...
	user : function( domain, options ){
		var pnoi = this.api.auth({key: this.creds.key ,secret: this.creds.secret, app: this.creds.app });
	...
	sub : function( domain, options ){
		// fallbacks
		options = options || {};
		var pnoi = this.api.auth({keyid: this.creds.key ,secret: this.creds.secret, app: this.creds.app });

		if( domain == "*" ){
			// read the domain list instead
			return pnoi.list(function( error, result ) {
		...
		pnoi.select( query, function( error, result ) {
			if( error ) return console.log( error );
			output( result );
		});
